...
def generate_bus_id():
    """Generate a random 6-character alphanumeric code for the Bus ID."""
...
class StudentProfile(models.Model):
    ...
    # id is auto BigInt PK (default Django behavior)
    # student_id is a separate unique CharField, not the primary key
    ...
    # New fields for RBAC and Wallet
    role = models.CharField(max_length=20, choices=ROLE_CHOICES, default='Student')
    wallet_balance = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
    profile_picture = models.URLField(blank=True, null=True)
    password = models.CharField(max_length=128, blank=True, null=True) # For staff login
...
class Bus(models.Model):
    ...
    def __str__(self):
        return f"Bus {self.bus_id_code} on {self.route.name}"
...
class Ticket(models.Model):
    ...
    # ForeignKey joins on StudentProfile.id (integer PK)
    ...
    @property
    def is_active(self):
        # Ticket is active for 1 hour after booking
...
class Notice(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(StudentProfile, on_delete=models.SET_NULL, null=True, limit_choices_to={'role': 'Vendor'})
